...
fetch('data.json')
  ...
  .then((result) => {
    ...
    document.querySelector('.container').addEventListener('click', (event) => {
      ...
      if (target && !target.classList.contains('specialized_keys')) {
    ...
    document.querySelector('.arrow_left').addEventListener('click', () => {
      changeCursorPosition(textArea.selectionStart - 1,
        textArea.selectionStart - 1, textArea.value);
    });
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (key.classList.contains('ctrl_left') || key.classList.contains('ctrl_right')) {
        ...
        let end;
        switch (true) {
          case !key.classList.contains('specialized_keys'):
            event.preventDefault();
            addSymbol(key.innerText);
            key.classList.add('active');
            break;
          case key.classList.contains('enter'):
            event.preventDefault();
            addSymbol('\n');
            key.classList.add('active');
            break;
          case key.classList.contains('tab'):
            event.preventDefault();
            addSymbol('\t');
            key.classList.add('active');
            break;
          case key.classList.contains('space'):
            event.preventDefault();
            addSymbol(' ');
            key.classList.add('active');
            break;
          case key.classList.contains('backspace'):
            event.preventDefault();
            changeCursorPosition(textArea.selectionStart - 1,
              textArea.selectionEnd, textArea.value);
            key.classList.add('active');
            break;
          case key.classList.contains('del'):
            event.preventDefault();
            changeCursorPosition(textArea.selectionStart,
              textArea.selectionEnd + 1, textArea.value);
            key.classList.add('active');
            break;
          case key.classList.contains('caps_lock') && !event.repeat:
            changeSymbol('.letter');
            key.classList.toggle('active');
            break;
          case key.classList.contains('alt_left'):
            event.preventDefault();
            key.classList.add('active');
            break;
          case key.classList.contains('alt_right'):
            event.preventDefault();
            key.classList.add('active');
            break;
          case key.classList.contains('arrow_left'):
            event.preventDefault();
            ...
            key.classList.add('active');
            break;
          case key.classList.contains('arrow_right'):
            event.preventDefault();
            start = textArea.selectionStart + 1;
            end = textArea.selectionEnd + 1;
            changeCursorPosition(start, end, textArea.value);
            key.classList.add('active');
            break;
          case (key.classList.contains('shift_left') || key.classList.contains('shift_right')) && !event.repeat:
            if (event.altKey) {
              changeLanguage();
              key.classList.add('active');
